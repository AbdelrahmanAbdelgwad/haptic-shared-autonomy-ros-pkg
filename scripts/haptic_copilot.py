...
def main(
    ...
    frames_per_state = 4
    model = DQNCopilot.load("Copilot_600k_noisy_fine_tuning", device="cpu")
    abs_timestep = 0
    for method in methods_schedule:
        for alpha in alpha_schedule:
            ...
            while not done:
                ...
                pi_action_steering_mapped = map_val(
                    disc_human_steering_action, -1, 1, 0, 255
                )

                pi_action_steering_frame = (
                    np.zeros(
                        (copilot_obs.shape[0], copilot_obs.shape[1]), dtype=np.int16
                    )
                ...
                copilot_obs[
                    :, :, 0:4
                ] = observation  # Copy the first four channels from observation
                copilot_obs[
                    :, :, 4
                ] = pi_action_steering_frame  # Assign pi_action_steering_frame to the fifth channel
                ...
                if feedback:
                    feedback_value = int(
                        get_feedback(disc_human_steering_action, opt_action)
                    )
